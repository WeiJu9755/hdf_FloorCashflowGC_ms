...
function processform($aFormValues){
	...
	$floor	= trim($aFormValues['floor']);
	
	if (trim($aFormValues['floor']) == "")	{
		$objResponse->script("jAlert('警示', '請輸入樓層', 'red', '', 2000);");
		return $objResponse;
		exit;
	}
	...
	if (!$bError) {
		...
		$Qry="insert into buildings_sub_detail (case_id,building,floor,makeby,last_modify) values ('$case_id','$building','$floor','$memberID',now())";
		$mDB->query($Qry);
		updateFloorCashflowGCModifyLog($mDB, $case_id, $memberID);
		//再取出auto_seq
		$Qry="select auto_seq from buildings_sub_detail where case_id = '$case_id' and building = '$building' and floor = '$floor' order by auto_seq desc limit 0,1";
		$mDB->query($Qry);
		if ($mDB->rowCount() > 0) {
			//已找到符合資料
			$row=$mDB->fetchRow(2);
		...
		if (!empty($auto_seq)) {
			...
			$objResponse->script("window.location='/?ch=buildings_sub_detail_modify&auto_seq=$auto_seq&fm=$fm';");
		} else {
		...
		<div id="info_container">
			<form method="post" id="addForm" name="addForm" enctype="multipart/form-data" action="javascript:void(null);">
				<div class="field_container3">
					<div>
						<div class="field_div1">請輸入樓層:</div> 
						<div class="field_div2">
							<input type="text" class="inputtext" id="floor" name="floor" size="20" style="width:100%;max-width:250px;" />
						</div> 
					</div>
				</div>
...
var myDraw = function(){
	var oTable;
	oTable = parent.$('#buildings_sub_detail_table').dataTable();
	oTable.fnDraw(false);
}
