...
function processform($aFormValues){
	...
	if (trim($aFormValues['building']) == "")	{
		$objResponse->script("jAlert('警示', '請輸入棟別', 'red', '', 2000);");
		return $objResponse;
		exit;
	}
	...
	/*
	//再取出auto_seq
	$Qry="select auto_seq from buildings_sub where case_id = '$case_id' and building = '$building' order by auto_seq desc limit 0,1";
	$mDB->query($Qry);
	if ($mDB->rowCount() > 0) {
		//已找到符合資料
		$row=$mDB->fetchRow(2);
	...
	if (!empty($auto_seq)) {
		...
		$objResponse->script("window.location='/?ch=buildings_sub_modify&auto_seq=$auto_seq&fm=$fm';");
	} else {
	...
	*/
	...
	$objResponse->script("art.dialog.tips('已新增!',1);");
	$objResponse->script("parent.$.fancybox.close();");
				...
				<div class="field_container3">
					<div>
						...
						<div class="field_div2">
							<input type="text" class="inputtext" id="building" name="building" size="20" style="width:100%;max-width:80px;" />
						</div> 
					</div>
					<div>
						...
						<div class="field_div2">
							<div class="input-group" id="scheduled_entry_date" style="width:100%;max-width:250px;">
								<input type="text" class="form-control" name="scheduled_entry_date" aria-describedby="scheduled_entry_date">
					...
					<div>
						...
						<div class="field_div2">
							<div class="input-group" id="actual_entry_date" style="width:100%;max-width:250px;">
								<input type="text" class="form-control" name="actual_entry_date" aria-describedby="actual_entry_date">
					...
					<div>
						...
						<div class="field_div2">
							<input type="text" class="inputtext" id="construction_days_first_floor" name="construction_days_first_floor" size="20" style="width:100%;max-width:80px;">
						</div> 
					</div>
					<div>
						<div class="field_div1">每層施作天數:</div> 
						<div class="field_div2">
							<input type="text" class="inputtext" id="construction_days_per_floor" name="construction_days_per_floor" size="20" style="width:100%;max-width:80px;">
						</div> 
					</div>
					<div>
						<div class="field_div1">標準層範圍:</div> 
						<div class="field_div2">
							<input type="text" class="inputtext" id="std_layer_floor" name="std_layer_floor" size="20" style="width:100%;max-width:400px;">
						</div> 
					</div>
					<div>
						<div class="field_div1">屋突層範圍:</div> 
						<div class="field_div2">
							<input type="text" class="inputtext" id="roof_protrusion_floor" name="roof_protrusion_floor" size="20" style="width:100%;max-width:400px;">
						</div> 
					</div>
					<div>
						<div class="field_div1">代工數:</div> 
						<div class="field_div2">
							<input type="text" class="inputtext" id="outsourcing" name="construction_days_fioutsourcingrst_floor" size="20" style="width:100%;max-width:80px;">
						</div> 
					</div>
				</div>
